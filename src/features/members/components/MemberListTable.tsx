    ...
    return members.filter((member) => {
      ...
      const matchVehicles = member.vehicles?.some(
        (v) =>
          ...
          v.plateProvince?.toLowerCase().includes(term),
      );
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6'>
        <div>
          ...
          <p className='text-slate-400 text-sm mt-1'>
            Manage registered member units, contact details, and associated vehicle plates.
          </p>
        </div>
      ...
      <div className='flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md'>
        ...
        <div className='relative flex-1'>
          ...
          <input
            ...
            placeholder='Search unit, telephone, vehicle plate, or brand...'
        ...
        <div className='flex items-center gap-3'>
          ...
          <select
            ...
            className='px-3.5 py-2 bg-slate-950/80 border border-slate-800 focus:border-indigo-500 rounded-xl text-slate-200 text-sm appearance-none transition-colors'
          >
      ...
      <div className='rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl overflow-hidden backdrop-blur-md'>
                    ...
                    >
                      ...
                      <td className='px-6 py-4'>
                        <Link
                          ...
                          className='font-semibold text-slate-100 group-hover:text-indigo-400 transition-colors flex items-center gap-1.5'
                        >
        ...
        <div className='px-6 py-4 bg-slate-950/60 border-t border-slate-800 text-xs text-slate-400 flex items-center justify-between'>
          <span>
            Showing <strong className='text-slate-200'>{filteredMembers.length}</strong> of{" "}
            <strong className='text-slate-200'>{members.length}</strong> member units
          </span>
          <span className='font-mono text-slate-500'>CARPARK API Integration</span>
        </div>
      </div>
